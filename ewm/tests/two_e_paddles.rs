...
fn iie_paddle_timer_charges_and_expires() {
    ...
    let (armed, later) = paddle_reads(TwoType::Apple2EEnhanced, 0, 3_000);
    ...
    let (_, left) = paddle_reads(TwoType::Apple2EEnhanced, i16::MIN, 200);
    assert_eq!(left, 0x00, "full-left expires quickly");
    let (_, right) = paddle_reads(TwoType::Apple2EEnhanced, i16::MAX, 2_000);
...
fn iie_paddles_match_the_2plus() {
    ...
    for axis in [i16::MIN, -12_345, 0, 12_345, i16::MAX] {
        for wait in [100, 1_000, 2_000, 4_000] {
            assert_eq!(
                paddle_reads(TwoType::Apple2EEnhanced, axis, wait),
